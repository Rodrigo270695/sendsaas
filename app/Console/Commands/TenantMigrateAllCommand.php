...
{
    protected $signature = 'sendsaas:tenant-migrate-all
                            {--slug= : Filtrar por slug del tenant}
                            {--schema= : Filtrar por nombre exacto del schema}
                            {--dry-run : Solo listar los schemas}
                            {--stop-on-error : Detener al primer fallo}';

    protected $description = 'Ejecuta migraciones tenant pendientes en todos los tenants.';
    ...
    public function handle(TenantSchemaMigrator $migrator): int
    {
        ...
        foreach ($tenants as $tenant) {
            $this->line(sprintf('  · %s → schema `%s` (%s)', $tenant->slug, $tenant->schema_name, $tenant->estado));
        }
        ...
        $this->newLine();
        if ($failures > 0) {
            $this->error('Terminado con '.$failures.' error(es).');
