...
export function DocumentTypeSelect({
    ...
    const labelFor = (code: string): string => {
        if (code === 'OTRO' || code === 'OTR') {
            return t('document_types.otr');
        }
        return t(`document_types.${code.toLowerCase()}`);
    };
    ...
    return (
        ...
        >
            ...
            >
                <SelectValue placeholder={t('document_types.placeholder')} />
            </SelectTrigger>
            <SelectContent>
                <SelectItem value={NONE_VALUE} className="cursor-pointer">
                    {t('document_types.placeholder')}
                </SelectItem>
                {codes.map((code) => (
