...
const DISMISS_KEY = 'sendsaas:impersonation-banner-minimized';

export function isTenantImpersonating(imp: unknown): imp is {
    tenant_label?: string;
    tenant_id?: string;
} {
    return Boolean(imp && typeof imp === 'object' && 'tenant_label' in imp);
}

/**
 * Aviso de modo soporte, igual que VetSaaS:
 * - Desktop: franja compacta encima de los breadcrumbs.
 * - Móvil: toast flotante abajo (portal a body para que overflow no lo recorte).
 */
export function TenantImpersonationBanner() {
    ...
    if (minimized) {
        ...
        return mounted ? createPortal(chip, document.body) : null;
    }
                ...
                <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 line-clamp-2 text-[0.7rem] leading-snug text-muted-foreground">
                            {t('impersonation.banner_body_short', {
                                clinic: label,
                            })}
                        </p>
                    </div>
    ...
    return (
        <>
            ...
            {mounted ? createPortal(mobileToast, document.body) : null}
        </>
    );
}
