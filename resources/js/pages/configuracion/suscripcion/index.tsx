...
import { OutboundQuotaCard } from './components/outbound-quota-card';
...
function urgencyHeroClass(urgency: SubscriptionSummary['urgency']): string {
    switch (urgency) {
        ...
        default:
            return 'from-brand-800 via-brand-600 to-brand-900';
    }
}
...
export default function Index({ subscription }: SuscripcionIndexProps) {
    ...
    const cicloLabel =
        summary?.ciclo !== null && summary?.ciclo !== undefined
            ...
            : '—';
    const soporteLabel = summary?.soporte_tipo
        ? t(`soporte.${summary.soporte_tipo}`, {
              defaultValue: summary.soporte_tipo,
          })
        : '—';
        ...
        <>
            ...
            <div className="flex flex-1 flex-col gap-5 p-4 sm:p-6">
                ...
                {summary && summary.urgency !== 'ok' && (
                    <Alert className={cn('py-2.5', urgencyAlertClass(summary.urgency))}>
                        ...
                        <AlertDescription className="flex flex-wrap items-center justify-between gap-2 text-sm">
                            <span>{t(`alerts.${summary.urgency}`)}</span>
                            {daysLabel ? (
                ...
                <div className="grid gap-5 xl:grid-cols-12">
                    <div className="overflow-hidden rounded-xl border border-border/60 bg-card/80 shadow-sm ring-1 ring-border/20 xl:col-span-8">
                        <div className="grid divide-y lg:grid-cols-3 lg:divide-x lg:divide-y-0">
                            <div className="p-4">
                                <PanelSection title={t('sections.plan')} icon={Package}>
                                    ...
                                    />
                                    <InfoRow
                                        label={t('fields.soporte')}
                                        value={soporteLabel}
                                    />
                                    <InfoRow
                                        label={t('fields.send_window')}
                                        value={
                                            summary
                                                ? `${summary.send_window.start} – ${summary.send_window.end}`
                                                : '—'
                                        }
                                        mono
                                    />
                    ...
                    <OutboundQuotaCard
                        sendWindowStart={summary?.send_window.start ?? '08:00'}
                        sendWindowEnd={summary?.send_window.end ?? '20:00'}
                        timezone={summary?.timezone ?? 'America/Lima'}
