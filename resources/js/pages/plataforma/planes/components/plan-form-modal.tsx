...
const buildInitialData = (plan: Plan | null): PlanFormData => ({
    ...
    badge: plan?.badge ?? '',
    color_hex: plan?.color_hex ?? '#AB3C3D',
...
const isFormValid = (data: PlanFormData): boolean => {
    ...
    if (data.precio_anual !== '' && Number.isNaN(Number(data.precio_anual))) {
        return false;
    }
    ...
    return (
        ...
        >
            <div className="flex flex-col gap-5">
                ...
                <FormSection
                    index={1}
                    title={t('planes:form.section_commercial')}
                    description={t('planes:form.section_commercial_hint')}
                >
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <FormField
