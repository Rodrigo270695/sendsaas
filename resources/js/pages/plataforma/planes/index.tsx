...
import { planIntFeature } from './types';
...
export default function Index({
    ...
    const {
        ...
        storageKey: 'sendsaas.plataforma.planes.prefs',
        defaults: {
    ...
    const columns = useMemo<DataTableColumn<Plan>[]>(() => {
        const base: DataTableColumn<Plan>[] = [
            ...
            {
                key: 'limits',
                header: t('planes:columns.limits'),
                cell: (plan) => {
                    const users = planIntFeature(plan, 'max_usuarios');
                    const sessions = planIntFeature(
                        plan,
                        'max_whatsapp_sessions',
                    );
                    const messages = planIntFeature(
                        plan,
                    ...
                    return (
                        <div className="flex flex-col gap-0.5 text-xs leading-tight">
                            <span className="text-foreground">
                                {formatLimit(
                                    users,
                                    (count) => t('planes:row.users', { count }),
                                    t('planes:row.users_unlimited'),
                                )}
                            </span>
                            <span className="text-muted-foreground">
                                {formatLimit(
                                    sessions,
                                    (count) =>
                                        t('planes:row.sessions', { count }),
                                    t('planes:row.sessions_unlimited'),
                                )}
                                {' · '}
                                {formatLimit(
                                    messages,
                                    (count) =>
                                        t('planes:row.messages_day', { count }),
                                    t('planes:row.messages_unlimited'),
                                )}
                            </span>
                        </div>
                    );
            ...
            <div className="flex flex-1 flex-col gap-5 p-4 sm:p-6">
                ...
                <DataTable
                    ...
                    emptyState={
                        <EmptyState
                            icon={
                                activeFiltersCount > 0 ? Activity : MessageCircle
                            }
                            title={
